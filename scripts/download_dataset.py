...
def download(dataset_id: int, output_dir: Path, log_file: Path):
    ...
    # download text labels first. Do not unzip
    helper.download_and_extract_file(dataset_key=71362, file_sn=train_label_keys, output_dir=output_dir, unzip=False)
    helper.download_and_extract_file(dataset_key=71362, file_sn=val_label_keys, output_dir=output_dir, unzip=False)
    ...
    for sn in tqdm(train_image_keys + val_image_keys, desc="Downloading and processing images"):
        ...
        helper.download_and_extract_file(dataset_key=71362, file_sn=sn, output_dir=output_dir, transform=resize_images_parallel)
        completed.append(str(sn))
        tqdm.write(f"Completed downloading and processing file_sn={sn}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log-file", type=str, default="./download_progress.txt", help="File to log download progress.")

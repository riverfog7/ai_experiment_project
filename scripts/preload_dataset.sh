...
TARBALL_URL="https://riverfog7.com/datasets/recyclables_image_classification.tar"
TARBALL_MD5="3ce49e0b6e7de63edb289f11fdf47b51"
...
if [ "$USE_ARIA2" = true ]; then
    ...
    aria2c -x 8 -s 8 -d "$DATASET_DIR" -o recyclables_image_classification.tar "$TARBALL_URL"
else
    echo "Downloading dataset using curl..."
    curl -L "$TARBALL_URL" -o "$DATASET_DIR/recyclables_image_classification.tar"
fi
...
DOWNLOADED_MD5=$(md5sum "$DATASET_DIR/recyclables_image_classification.tar" | awk '{ print $1 }')
...
tar -xf "$DATASET_DIR/recyclables_image_classification.tar" -C "$DATASET_DIR"
if [ $? -ne 0 ]; then
    echo "Error: Failed to extract dataset."
    exit 1
fi
rm "$DATASET_DIR/recyclables_image_classification.tar"
